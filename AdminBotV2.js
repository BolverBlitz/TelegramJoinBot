...
bot.on(/^\/check$/i, (msg) => {
	...
	Promise.all([SW.getBan(UserID, 0),SW.getBan(UserID, 1), SW.getBan(UserID, 2), AntiSpam.checkUserCAS(UserID), AntiSpam.checkUserspamprotection(UserID), AntiSpam.checkSpamblockers(UserID)]).then(function(PAll) {
	//Promise.all([SW.getBan(UserID, 0),SW.getBan(UserID, 1), SW.getBan(UserID, 2), AntiSpam.checkUserCAS(UserID)]).then(function(PAll) {
		...
		bot.sendMessage(msg.chat.id, i18n(config.language, "AntiSpamCheck", { Nutzername: username, Spamwatch: f.ConvertBoolToEmoji(PAll[0].state), EBGWatch: f.ConvertBoolToEmoji(PAll[1].state), BolverWatch: f.ConvertBoolToEmoji(PAll[2].state), CAS:f.ConvertBoolToEmoji(PAll[3].state), spamprotection: f.ConvertBoolToEmoji(PAll[4].state), Spamblockers: f.ConvertBoolToEmoji(PAll[5].state)}) + BanMSGState, {webPreview: false})
...
bot.on('callbackQuery', (msg) => {
	...
	if(data[0] === 'gban')
	{
		if(data[2] === userID){
			...
			if(data[1] === 'check')
			{
				Promise.all([SW.getBan(data[3], 0),SW.getBan(data[3], 1), SW.getBan(data[3], 2), AntiSpam.checkUserCAS(data[3]), AntiSpam.checkUserspamprotection(data[3]), AntiSpam.checkSpamblockers(data[3])]).then(function(PAll) {
					let BanMSGState = "";
					PAll.map(bool => {
						if(bool.state === true){
								if(bool.reason.startsWith("0x")){
					...
					let MSG = i18n(config.language, "AntiSpamCheck", { Nutzername: data[4], Spamwatch: f.ConvertBoolToEmoji(PAll[0].state), EBGWatch: f.ConvertBoolToEmoji(PAll[1].state), BolverWatch: f.ConvertBoolToEmoji(PAll[2].state), CAS:f.ConvertBoolToEmoji(PAll[3].state), spamprotection: f.ConvertBoolToEmoji(PAll[4].state), Spamblockers: f.ConvertBoolToEmoji(PAll[5].state)}) + BanMSGState
